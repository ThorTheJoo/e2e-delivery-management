    ...
    function notify(message, type = 'info', timeout = 3000){
        ...
        Object.assign(n.style, {
            ...
            color: '#fff', padding: '8px 12px', borderRadius: '8px', zIndex: 99999
        });
        ...
        sidebarMenu.addEventListener('click', function(e) {
            ...
            if (target) {
                ...
                if (children) {
                    ...
                    console.log('Toggled expanded state for:', section, 'Was expanded:', wasExpanded, 'Now expanded:', target.classList.contains('expanded'));
                } else {
            ...
            if (childTarget) {
                ...
                if (tabId) {
                    const tabButton = document.querySelector(`.top-tabs-section .tab-btn[data-tab="${tabId}"]`);
                    if (tabButton) {
                        tabButton.click();
                    }
                }
    ...
    function initCollapsibles(){
        document.querySelectorAll('.section-collapsible').forEach(section => {
            ...
            const header = section.querySelector('.panel-header') || section.querySelector('.capability-header');
            const toggle = document.createElement('button');
            toggle.type = 'button'; toggle.className = 'collapse-toggle in-header';
    ...
    async function loadBOMPreview(){
        ...
        try{
            ...
            container.innerHTML = '<div class="muted">Unable to load demo-data.json (check file exists)</div>';
            console.error(err);
        }
    }
    ...
    function renderBOMPreview(container, data){
        ...
        const html = [];
        html.push(`<div class="bom-card"><h4>Product: ${escapeHtml(product.name)}</h4><p class="muted">${escapeHtml(product.description||'')}</p></div>`);
        ...
        if(services.length === 0) html.push('<div class="muted">No services in demo data</div>');
            ...
            html.push('<ul>');
            services.forEach(s => html.push(`<li><strong>${escapeHtml(s.name)}</strong> — ${escapeHtml(s.phase||'')} — ${escapeHtml(String(s.effort||''))} PD — ${escapeHtml(String(s.cost||''))}</li>`));
            html.push('</ul>');
        }
        html.push('</div>');
        ...
        if(resources.length === 0) html.push('<div class="muted">No resources in demo data</div>');
            ...
            html.push('<ul>');
            resources.forEach(r => html.push(`<li><strong>${escapeHtml(r.name)}</strong> — ${escapeHtml(r.type||'')} — ${escapeHtml(String(r.rate||''))}</li>`));
            html.push('</ul>');
        }
        html.push('</div>');
    ...
    function initButtons(){
        document.getElementById('generateBOM')?.addEventListener('click', ()=> { loadBOMPreview(); notify('BOM preview generated', 'success'); });
        document.getElementById('exportBOM')?.addEventListener('click', ()=> { notify('Export not configured in demo', 'info'); });
        document.getElementById('downloadBOM')?.addEventListener('click', ()=> { notify('Download started (demo)', 'info'); });
    }
    ...
    function updateDomainCounts(){
        document.querySelectorAll('.domain-group').forEach(group => {
            const count = group.querySelectorAll('.domain-capabilities input[type=checkbox]:checked').length;
...
function renderCurrentWorkflowStatus(){
    ...
    html.push('<div class="milestone completed"><i class="fas fa-check-circle"></i><span>Presales Complete</span><span class="milestone-date">Dec 2024</span></div>');
    html.push('<div class="milestone in-progress"><i class="fas fa-clock"></i><span>Delivery Definition</span><span class="milestone-date">Jan 2025</span></div>');
    html.push('<div class="milestone pending"><i class="fas fa-hourglass-start"></i><span>Build Complete</span><span class="milestone-date">Q2 2025</span></div>');
    html.push('<div class="milestone pending"><i class="fas fa-hourglass-start"></i><span>Production Cutover</span><span class="milestone-date">Q3 2025</span></div>');
    html.push('</div></div>');
    html.push('</div>');
...
function updateDomainFilterOptionsFromSet(domains){
    ...
    Array.from(merged).filter(Boolean).sort().forEach(d => {
        const val = slugify(d);
        const opt = document.createElement('option');
        opt.value = val; opt.textContent = d;
        sel.appendChild(opt);
    });
}
...
function parseTMFCategoriesFromWorkbook(workbook){
    const sheetName = (workbook.SheetNames || []).find(n => /input\s*-\s*tmf\s*categories/i.test(String(n)));
    ...
    const out = {};
    Object.keys(domains).forEach(d => { out[d] = Array.from(domains[d]); });
    return out;
}
...
function updateCapabilityRequirementBadges(countsByCapability) {
    ...
    Object.entries(countsByCapability).forEach(([capId, count]) => {
        const label = document.querySelector(`.capability-item label[for="${capId}"]`) || document.querySelector(`.capability-item label[for="capability-${capId}"]`);
        if (label) {
...
function renderImportedRequirementsPanel(state, mapping) {
    const scopingPanel = document.querySelector('.scoping-panel') || document.getElementById('capabilities');
    ...
    const domains = Object.entries(state.counts.domains || {}).map(([k,v]) => `${k}: ${v}`).join(', ');
    const caps = Object.entries(mapping.countsByCapability || {}).map(([k,v]) => `${k}: ${v}`).join(', ');
    const list = mapping.assignments.slice(0, 10).map(a => `<li><strong>${a.requirementId || 'N/A'}</strong> → <code>${a.capabilityId}</code></li>`).join('');
    panel.innerHTML = `
...
function parseCSVToSpecSyncItems(text){
    ...
    for(let r = 1; r < lines.length; r++){
        ...
        cols.push(cur);
        const get = (i)=> i>=0 ? String(cols[i] || '').trim().replace(/^"|"$/g,'') : '';
        const functionName = get(iFunc);
        const afLevel2 = get(iAf2);
        const referenceCapability = get(iCap);
        const tagVals = [];
        if(iTags>=0) tagVals.push(get(iTags));
        iCommon.forEach(i => { if(i>=0) tagVals.push(get(i)); });
        iAdditional.forEach(i => { if(i>=0) tagVals.push(get(i)); });
        const tags = Array.from(new Set(tagVals
...
function initializeSpecSyncIntegration(){
    ...
    if(importBtn){
        importBtn.addEventListener('click', async ()=>{
            ...
            if(!file){ if(window.showNotification) showNotification('Select a SpecSync file first','info'); return; }
            // Ensure Excel reader is present if importing xlsx
            if(/\.xlsx?$/i.test(file.name)){
                const ok = await ensureXLSX();
                if(!ok){ throw new Error('Excel reader not available (CDN blocked). Please import CSV/JSON instead.'); }
            }
            ...
            reader.onload = ()=>{
                try{
                    ...
                    if(/\.json$/i.test(file.name)){
                        ...
                        const preferred = (workbook.SheetNames || []).find(n => /rephrased|atomic/i.test(String(n))) || workbook.SheetNames[0];
                        ...
                        try{
                            ...
                            if(tmfCategories && Object.keys(tmfCategories).length){
                                ...
                                const importedDomains = new Set([...(items.map(it => (it.domain||'Imported').toString().trim() || 'Imported')),...Object.keys(tmfCategories)]);
                                updateDomainFilterOptionsFromSet(importedDomains);
                            }
                        }catch(e){ console.warn('TMF categories parsing failed', e); }
                    }else{
                    ...
                    const importedDomains = new Set(items.map(it => (it.domain||'Imported').toString().trim() || 'Imported'));
                    updateDomainFilterOptionsFromSet(importedDomains);
                    ...
                    const topCaps = Object.entries(dyn.countsByCapability).sort((a,b)=>b[1]-a[1]).slice(0,2).map(([k,v])=>`${k}(${v})`).join(', ');
                ...
                }catch(err){ console.error('SpecSync import error:', err); if(window.showNotification) showNotification(`Failed to import SpecSync: ${err.message||'Error'}`,'error'); }
            };
            reader.onerror = ()=>{ if(window.showNotification) showNotification('Failed to read file','error'); };
    ...
    if(clearBtn){
        clearBtn.addEventListener('click', ()=>{
            try{ localStorage.removeItem('specsync-data'); }catch(e){}
...
async function ensureXLSX(){
    ...
    const trySrc = (src)=> new Promise((resolve)=>{
        ...
        setTimeout(()=>{ if(!done){ done = true; resolve(typeof XLSX !== 'undefined'); } }, 2000);
    });
...
function bindSpecSyncDelegatedHandlers(){
    ...
    document.addEventListener('click', async (e)=>{
        const importBtn = e.target && (e.target.id === 'importSpecSync' ? e.target : e.target.closest && e.target.closest('#importSpecSync'));
        const clearBtn = e.target && (e.target.id === 'clearSpecSync' ? e.target : e.target.closest && e.target.closest('#clearSpecSync'));
        if(importBtn){
            try{
                const fileInput = document.getElementById('specSyncFile');
                const file = fileInput && fileInput.files && fileInput.files[0];
                ...
                if(/\.xlsx?$/i.test(file.name)){
                    const ok = await ensureXLSX();
                    if(!ok){ if(window.showNotification) showNotification('Excel reader not available (CDN blocked). Try CSV/JSON.','error'); return; }
                }
                const reader = new FileReader();
                reader.onload = ()=>{
                    try{
                        ...
                        if(/\.json$/i.test(file.name)){
                            ...
                            items = json.map((r, idx) => {
                                ...
                                for(let i=2;i<=5;i++){ const v = r[`Common Tag ${i}`] || r[`CommonTag${i}`]; if(v) tagCandidates.push(String(v)); }
                                for(let i=1;i<=10;i++){ const v = r[`Additional Tag ${i}`] || r[`AdditionalTag${i}`]; if(v) tagCandidates.push(String(v)); }
                                ...
                                const tags = Array.from(new Set(tagCandidates.flatMap(v=>String(v||'').split(/[,;|]/)).map(s=>s.trim()).filter(Boolean)));
                                const af2 = r['Rephrased AF Lev.2'] || r['Rephrased AF Lev. 2'] || r['AF Lev.2'] || r['AF Level 2'] || r['Architecture Framework Level 2'] || '';
                                return {
                                    rephrasedRequirementId: r['Rephrased Requirement ID'] || r['RephrasedRequirementId'] || r['RequirementId'] || '',
                                    sourceRequirementId: r['Source Requirement ID'] || r['SourceRequirementId'] || '',
                                    domain: r['Rephrased Domain'] || r['Domain'] || '',
                                    vertical: r['Rephrased Vertical'] || r['Vertical'] || '',
                                    functionName: r['Rephrased Function Name'] || r['Function Name'] || r['Function'] || '',
                                    afLevel2: af2,
            ...
            }catch(err){ console.error('SpecSync delegated import error:', err); if(window.showNotification) showNotification(`Failed to import SpecSync: ${err.message||'Error'}`,'error'); }
        } else if(clearBtn){
            try{ localStorage.removeItem('specsync-data'); }catch(e){}
            resetSpecSyncUI((t)=>{ const s=document.getElementById('specSyncSummary'); if(s) s.textContent = t || ''; });
            if(window.showNotification) showNotification('Cleared SpecSync import','info');
        }
    });
}
...
function resetSpecSyncUI(setSummary){
    ...
    document.querySelectorAll('.capability-item .req-badge').forEach(el => el.remove());
    document.querySelectorAll('.capability-item input[type="checkbox"]').forEach(cb => cb.checked = false);
    const panel = document.getElementById('importedRequirementsPanel');
    if(panel) panel.remove();
...
function generateBOMCSV(bomData) {
    ...
    const risks = [
        ['R001', 'Technical', 'Integration complexity with legacy systems', 'Medium', 'High', 'High', 'Early prototyping and proof of concept', 'Additional integration specialists', 'Technical Lead'],
        ['R002', 'Schedule', 'Resource availability during peak periods', 'Medium', 'Medium', 'Medium', 'Resource planning and backup resources', 'Extended timeline or additional resources', 'Project Manager'],
        ['R003', 'Data', 'Data migration complexity and quality issues', 'High', 'High', 'High', 'Comprehensive data profiling and validation', 'Phased migration approach', 'Data Architect'],
        ['R004', 'Business', 'Stakeholder alignment and requirement changes', 'Medium', 'High', 'High', 'Regular stakeholder communication and change management', 'Scope freeze and change control process', 'Business Analyst'],
        ['R005', 'Technical', 'Performance issues with high transaction volumes', 'Low', 'High', 'Medium', 'Performance testing and optimization', 'Infrastructure scaling', 'Performance Engineer']
    ];
    ...
    const qualityGates = [
        ['QG001', 'Requirements Review', 'Presales', 'All requirements documented and approved', 'Business Analyst, Stakeholders', 'Yes', 'Pending'],
        ['QG002', 'Architecture Review', 'Design', 'Architecture design approved by technical committee', 'Solution Architect, Technical Lead', 'Yes', 'Pending'],
        ['QG003', 'Code Review', 'Build', 'All code reviewed and approved', 'Senior Developer, Technical Lead', 'Yes', 'Pending'],
        ['QG004', 'Test Completion', 'Testing', 'All test cases executed and passed', 'QA Lead, Test Manager', 'Yes', 'Pending'],
        ['QG005', 'Go-Live Readiness', 'Deployment', 'All pre-go-live criteria met', 'Project Manager, Operations Team', 'Yes', 'Pending']
    ];
...
function generateCustomFields() {
    return {
        ...
        'Custom.ServicePhase': {
            ...
            allowedValues: ['Presales', 'Design', 'Build', 'Testing', 'Migration', 'Deployment', 'Post-Deployment', 'Warranty']
        },
        'Custom.Effort': {
            type: 'String',
...
function initializeWorkflowTabs() {
    ...
    setTimeout(() => {
        ...
        const tabToolsContentContainer = document.querySelector('.tab-content-area .tools-content');
            ...
            ['workflow-status', 'capabilities', 'inventory', 'estimates', 'ado-integration'].forEach((tabId) => {
                ...
                if (source && !source.closest('.tab-content-area')) {
                    try {
                        console.log(`Cloning tab content (found ${source.id}) into main tab panel as #${tabId}`);
                    ...
                    console.warn(`No source found for tab '${tabId}' (tried '${tabId}', '${tabId}-legacy')`);
                }
            });

            // Also de-duplicate any default tabs that might still be present outside the panel
            ['workflow-status', 'workflow-status-legacy', 'dependencies', 'dependencies-legacy', 'milestones', 'milestones-legacy', 'risks', 'integrations', 'integrations-legacy', 'inventory-legacy'].forEach((tabId) => {
            ...
            Array.from(document.querySelectorAll('h3')).forEach((heading) => {
                ...
                if (text.includes('Current Workflow Status') && !heading.closest('.tab-content-area')) {
                    const container = heading.closest('.tab-content') || heading.closest('section') || heading.parentElement;
                    if (container) {
...
function resetWorkflow() {
    if (confirm('Are you sure you want to reset the workflow progress? This action cannot be undone.')) {
        const progressBars = document.querySelectorAll('.progress-fill');
        const progressValues = document.querySelectorAll('.summary-value');
...
function testTabFunctionality() {
    ...
    tabButtons.forEach((btn, i) => console.log(`  Button ${i}: ${btn.getAttribute('data-tab')} - ${btn.textContent.trim()}`));
    
    // Test 2: Check if tab contents exist
    const tabContents = document.querySelectorAll('.tab-content-area .tools-content .tab-content');
    console.log('Tab contents found:', tabContents.length);
    tabContents.forEach((content, i) => console.log(`  Content ${i}: ${content.id} - Active: ${content.classList.contains('active')} - Display: ${getComputedStyle(content).display}`));
    ...
    tabButtons.forEach((button, index) => {
        ...
        if (content) {
            console.log(`  Content display: ${getComputedStyle(content).display}, visibility: ${getComputedStyle(content).visibility}`);
        }
    });
    
    // Test 4: Check current active state
    const activeButton = document.querySelector('.top-tabs-section .tools-tabs .tab-btn.active');
    const activeContent = document.querySelector('.workflow-tools-panel .tools-content .tab-content.active');
