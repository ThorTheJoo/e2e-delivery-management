  ...
  try {
    ...
    try {
      ...
      return NextResponse.json(
        {
          success: false,
          error: `Service creation failed: ${serviceError instanceof Error ? serviceError.message : 'Unknown error'}`,
        },
        { status: 500 },
      );
    }
      ...
      case 'test-connection':
        try {
          ...
          if (config.protocol === 'ODATA') {
            ...
            const baseUrl = config.odataUrl.endsWith('/')
              ? config.odataUrl.slice(0, -1)
              : config.odataUrl;
            ...
            if (baseResponse.status === 401) {
              ...
              console.log(
                '❌ Base endpoint not reachable:',
                baseResponse.status,
                baseResponse.statusText,
              );
              throw new Error(
                `OData endpoint not reachable: ${baseResponse.status} ${baseResponse.statusText}`,
              );
            } else {
            ...
            if (config.apiKey) {
              ...
              throw new Error(
                'OData authentication required: Please provide either API key or username/password',
              );
            }
            ...
            if (!authResponse.ok) {
              ...
              throw new Error(
                `OData authentication failed: ${authResponse.status} ${authResponse.statusText} - ${errorText}`,
              );
            }
            ...
            try {
              ...
              if (serviceRootResponse.ok) {
                ...
                console.log(
                  '❌ Service root failed:',
                  serviceRootResponse.status,
                  serviceRootResponse.statusText,
                );
              }
            } catch (error) {
            ...
            try {
              ...
              if (objectsResponse.ok) {
                ...
                console.log(
                  '❌ Objects (no auth) failed:',
                  objectsResponse.status,
                  objectsResponse.statusText,
                );
              }
            } catch (error) {
      ...
      case 'get-objects':
        try {
          ...
          if (filter) {
            try {
              ...
              if (
                ...
                throw new Error(
                  'Invalid OData filter syntax. Please use proper OData expressions or text search.',
                );
              }
              ...
              throw new Error(
                `OData filter validation failed: ${filterError instanceof Error ? filterError.message : 'Invalid filter syntax'}`,
              );
            }
          }
          ...
          if (config.apiKey) {
            ...
            console.log('❌ No authentication credentials provided');
            throw new Error(
              'Authentication required: Please provide either API key or username/password',
            );
          }
      ...
      case 'test-odata-functions':
        try {
          ...
          for (const testFunc of testFunctions) {
            try {
              ...
              const response = await fetch(
                `${config.odataUrl}/Objects?$filter=${encodeURIComponent(testFunc.filter)}&$top=1`,
                {
                  headers: {
                    Accept: 'application/json',
                    'OData-MaxVersion': '4.0',
              ...
              if (response.ok) {
                ...
                console.log(
                  `❌ Function ${testFunc.name} failed: ${response.status} - ${errorText}`,
                );
              }
            } catch (error) {
              testResults.failedFunctions.push({
                name: testFunc.name,
          ...
          return NextResponse.json(
            {
              success: false,
              error: error instanceof Error ? error.message : 'Function testing failed',
            },
            { status: 500 },
          );
        }
        ...
        try {
          ...
          for (const discoveryEndpoint of discoveryEndpoints) {
            try {
              ...
              if (response.ok) {
                investigationResults.discoveryEndpoints =
                  investigationResults.discoveryEndpoints || [];
          ...
          for (let i = 0; i < endpointsToTest.length; i += batchSize) {
            ...
            console.log(
              `Batch ${Math.floor(i / batchSize) + 1} completed. Found ${batchResults.filter((r) => r.success).length} available endpoints.`,
            );
          }
        ...
        try {
          ...
          if (filter && typeof filter === 'string') {
            ...
            if (!/^[a-zA-Z0-9\s_\-\.\(\)\=\'\&\|\,\<\>\"\s]+$/.test(filter)) {
              console.warn(
                '⚠️ [Blue Dolphin] Filter validation warning - potentially unsafe characters:',
                filter,
              );
          ...
          console.log(
            '🔑 [Blue Dolphin] Using authentication:',
            config.apiKey ? 'API Key' : 'Basic Auth',
          );
          ...
          if (result.value && result.value.length > 0) {
            ...
            const enhancedFields = Object.keys(firstObj).filter(
              (key) =>
            ...
            console.log(
              '📋 [Blue Dolphin] Sample enhanced field values:',
